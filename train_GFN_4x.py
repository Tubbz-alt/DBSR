...
def mkdir_steptraing():
    ...
    step1_folder, step2_folder, step3_folder = join(models_folder, '1'), join(models_folder, '2'), join(models_folder,
                                                                                                        '3')
...
def train(train_gen, model, criterion, optimizer, epoch, opt):
    epoch_loss = 0
    for iteration, batch in enumerate(train_gen, 1):
        # input, targetdenoise, targetdeblur, targetsr
        LR_Blur_Noise = batch[0].to(device)
        LR_Blur_Denoise = batch[1].to(device)
        LR_Deblur_Noise = batch[2].to(device)
        HR = batch[3].to(device)
        ...
        [lr_denoise, lr_deblur, sr] = model(LR_Blur_Noise, gated_Tensor, test_Tensor)

        loss1 = criterion(lr_denoise, LR_Blur_Denoise)
        loss2 = criterion(lr_deblur, LR_Deblur_Noise)
        loss3 = criterion(sr, HR)
        mse = loss3 + opt.lambda_db * (loss1 + loss2)
        epoch_loss += mse
        optimizer.zero_grad()
        mse.backward()
        optimizer.step()
    ...
    for i in range(opt.start_training_step, 4):
        ...
        for epoch in tqdm(range(opt.start_epoch, opt.nEpochs + 1)):
            ...
            for j in range(len(train_sets)):
                ...
                train(trainloader, model, criterion, optimizer, epoch, opt)
            checkpoint(i, epoch)
