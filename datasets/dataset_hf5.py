...
class DataSet(data.Dataset):
    def __init__(self, h5py_file_path):
        ...
        self.denoises = self.file.get("label_dn")
    ...
    def __getitem__(self, index):
        ...
        denoise_patch = np.asarray(self.denoises[index, :, :, :], np.float32)
        ...
        if random.randint(0, 1) == 0:
            ...
            denoise_patch = np.flip(denoise_patch, 2)
        ...
        denoise_patch = np.rot90(denoise_patch, rotation_times, (1, 2))
        deblur_patch = np.rot90(deblur_patch, rotation_times, (1, 2))
        hr_patch = np.rot90(hr_patch, rotation_times, (1, 2))

        return input_patch.copy(), denoise_patch.copy(), deblur_patch.copy(), hr_patch.copy()
